...
if (process.env.GEMINI_API_KEY) {
  ...
  console.log("🚀 Gemini API client initialized (Google AI Studio).");
}
...
if (process.env.GROQ_API_KEY) {
  ...
  console.log("🚀 Groq API client initialized (OpenAI-compatible).");
}
...
if (!process.env.GEMINI_API_KEY && !process.env.GROQ_API_KEY && !process.env.OPENAI_API_KEY) {
  console.warn("⚠️ Warning: No AI provider keys (GEMINI_API_KEY, GROQ_API_KEY, or OPENAI_API_KEY) defined in the environment. AI extraction will use local heuristics.");
}

/**
 * Retries a promise-returning function up to maxRetries times with exponential backoff.
 */
async function retryWithBackoff(fn, providerName = "API", retries = 3, delay = 1000) {
  try {
    return await fn();
  } catch (error) {
    if (retries <= 0) {
    ...
    console.warn(`⚠️ ${providerName} API call failed. Retrying in ${delay}ms... (${retries} attempts left). Error: ${error.message}`);
    await new Promise((resolve) => setTimeout(resolve, delay));
    return retryWithBackoff(fn, providerName, retries - 1, delay * 2);
  }
}
...
function extractLeadsHeuristically(headers, rows) {
  ...
  // Identify secondary email and phone columns
  const primaryEmailHeader = mappings.email;
  const primaryPhoneHeader = mappings.mobile_without_country_code;
  
  const secondaryEmailHeaders = [];
  const secondaryPhoneHeaders = [];
  
  for (const h of headers) {
    if (h === primaryEmailHeader || h === primaryPhoneHeader) continue;
    const cleaned = h.toLowerCase().replace(/[\s_-]+/g, ' ').replace(/[^\w\s]/g, '').trim();
    
    // Check if cleaned header contains any synonym for email
    const isEmailSyn = synonyms.email.some(syn => cleaned === syn || cleaned.includes(syn) || syn.includes(cleaned));
    if (isEmailSyn) {
      secondaryEmailHeaders.push(h);
      continue;
    }
    
    // Check if cleaned header contains any synonym for phone/mobile
    const isPhoneSyn = synonyms.mobile_without_country_code.some(syn => cleaned === syn || cleaned.includes(syn) || syn.includes(cleaned));
    if (isPhoneSyn) {
      secondaryPhoneHeaders.push(h);
    }
  }
  ...
  for (const row of rows) {
    ...
    // Extract secondary emails/phones from other columns
    const extraEmails = [];
    for (const h of secondaryEmailHeaders) {
      if (row[h]) {
        const val = String(row[h]).trim();
        if (val && val !== lead.email && !extraEmails.includes(val)) {
    ...
    const extraPhones = [];
    for (const h of secondaryPhoneHeaders) {
      if (row[h]) {
        const val = String(row[h]).trim();
        if (val && val !== lead.mobile_without_country_code && !extraPhones.includes(val)) {
    ...
    if (rawPhone) {
      ...
      if (rawPhone.startsWith('+')) {
        ...
        if (match) {
          ...
          lead.mobile_without_country_code = match[2].trim();
        }
      } else if (rawPhone.startsWith('91') && rawPhone.length > 10) {
        lead.country_code = "+91";
        lead.mobile_without_country_code = rawPhone.substring(2).trim();
      } else {
        lead.mobile_without_country_code = rawPhone.trim();
      }
    }
    ...
    // Keep crm_note clean and append parsed secondary contact items
    let crmNoteText = "";
    if (row.Notes || row.notes || row.Notes) {
      crmNoteText = String(row.Notes || row.notes || "").trim();
    } else if (lead.crm_note) {
      crmNoteText = lead.crm_note.trim();
    }
    
    if (extraEmails.length > 0) {
      const emailNote = `[Extra Emails: ${extraEmails.join(', ')}]`;
      crmNoteText = crmNoteText ? `${crmNoteText} | ${emailNote}` : emailNote;
    }
    if (extraPhones.length > 0) {
      const phoneNote = `[Extra Phone Numbers: ${extraPhones.join(', ')}]`;
      crmNoteText = crmNoteText ? `${crmNoteText} | ${phoneNote}` : phoneNote;
    }
    lead.crm_note = crmNoteText;
...
/**
 * Sends a batch of raw CSV rows to the active LLM provider (Groq, Gemini, or OpenAI)
 * for semantic mapping and field extraction.
...
`;
  ...
  // 1. ROUTING LOGIC: Groq -> Gemini -> OpenAI -> Local Fallback
  if (groq) {
    console.log("🤖 Routing batch to Groq (llama-3.1-8b-instant)...");
    try {
      const apiCall = () => groq.chat.completions.create({
        model: 'llama-3.1-8b-instant',
        messages: [
          { role: 'system', content: 'You are a precise CRM data parsing service. You output valid JSON objects matching the requested schema and filter rules.' },
      ...
      const completion = await retryWithBackoff(apiCall, "Groq");
      ...
      console.warn("⚠️ Groq extraction failed, falling back to next available provider. Error:", err.message);
    }
  }

  if (genAI) {
    console.log("🤖 Routing batch to Google Gemini (gemini-1.5-flash)...");
    try {
      const apiCall = async () => {
      ...
      const responseText = await retryWithBackoff(apiCall, "Gemini");
      const parsed = JSON.parse(responseText);
      if (parsed && Array.isArray(parsed.leads)) {
        return parsed.leads;
      }
    } catch (err) {
      console.warn("⚠️ Gemini extraction failed, falling back to next available provider. Error:", err.message);
    }
  }

  if (openai) {
    console.log("🤖 Routing batch to OpenAI (gpt-4o-mini)...");
    try {
      ...
      const completion = await retryWithBackoff(apiCall, "OpenAI");
      const responseText = completion.choices[0].message.content;
      const parsed = JSON.parse(responseText);
      if (parsed && Array.isArray(parsed.leads)) {
        return parsed.leads;
      ...
      console.warn("⚠️ OpenAI extraction failed. Error:", err.message);
    }
  }
