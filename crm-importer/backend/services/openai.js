...
if (apiKey) {
  ...
  console.warn("⚠️ Warning: OPENAI_API_KEY is not defined in the environment. AI extraction will use local heuristics.");
}
...
async function retryWithBackoff(fn, retries = 3, delay = 1000) {
  try {
    ...
    console.warn(`⚠️ OpenAI API call failed. Retrying in ${delay}ms... (${retries} attempts left). Error: ${error.message}`);
  ...
  for (const row of rows) {
    ...
    if (rawPhone) {
      ...
      if (rawPhone.startsWith('+')) {
        // Extract country code from string like "+91-987654" or "+91 98765" or "+1(555)"
    ...
    if (sourceVal.includes("demand")) {
      ...
      // Try to check note or description or campaign
...
/**
 * Sends a batch of raw CSV rows to OpenAI for semantic mapping and field extraction.
...
async function extractLeadsBatch(headers, rows) {
  if (!openai) {
    console.warn("⚠️ OpenAI not initialized. Using local heuristics.");
...
11. crm_status: Status of the lead. MUST be strictly one of these values (case-sensitive):
    ...
    - BAD_LEAD
    - SALE_DONE
    If not specified or unclear, classify based on notes or status description, or default to GOOD_LEAD_FOLLOW_UP.
  ...
  try {
    const apiCall = () => openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a precise CRM data parsing service. You output valid JSON objects matching the requested schema and filter rules.'
        },
        {
          role: 'user',
          content: prompt
    ...
    if (parsed && Array.isArray(parsed.leads)) {
      ...
      console.warn("⚠️ OpenAI response did not contain 'leads' array:", responseText);
      return extractLeadsHeuristically(headers, rows);
    }
  } catch (error) {
    console.error("⚠️ OpenAI batch processing failed. Falling back to heuristic mapping. Error:", error.message);
    return extractLeadsHeuristically(headers, rows);
  }
}
